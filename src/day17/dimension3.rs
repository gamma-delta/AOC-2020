...
impl Dimension {
    ...
    pub fn new(i: &str) -> Result<Self> {
        ...
        for (y, line) in i.lines().enumerate() {
            for (x, chr) in line.chars().enumerate() {
                ...
                if active {
                    world.insert(Coord3::new(x as i32, y as i32, 0));
                }
            }
        }
    ...
    pub fn get_world(&self) -> &HashSet<Coord3> {
        &self.world
    }
}
...
/// Iterator for a Coord3's neighbors. Iteration order is undefined.
